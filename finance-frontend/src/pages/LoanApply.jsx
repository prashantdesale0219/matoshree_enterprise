...
const LoanApply = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto py-12 px-4 animate-in fade-in zoom-in duration-500">
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-cosmic p-10 md:p-16 text-center space-y-8 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-secondary/5 rounded-bl-full -mr-16 -mt-16" />
          
          <div className="relative mx-auto w-24 h-24 bg-secondary/10 rounded-3xl flex items-center justify-center text-secondary mb-6 rotate-3">
            <AlertCircle size={48} />
          </div>
          ...
          <button 
            ...
            className="px-10 py-4 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl active:scale-95"
          >
            Back to Dashboard
          </button>
        </div>
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (step < 4) {
      // Validate live photo on step 1
      if (step === 1 && !formData.livePhoto) {
        alert('Please capture a live photo before proceeding. It is compulsory.');
        return;
      }
      setStep(step + 1);
      return;
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        // Update local user profile with the live photo
        if (formData.livePhoto) {
          updateUserProfile({ profilePic: formData.livePhoto });
        }
    ...
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="text-center md:text-left space-y-2">
        ...
        <h1 className="text-4xl font-black text-primary tracking-tight">
          Apply for <span className="text-secondary">Finance</span>
        </h1>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-cosmic relative overflow-hidden">
        <div className="flex items-center justify-between relative px-2 md:px-10">
          {steps.map((s) => (
            <div key={s.number} className="flex flex-col items-center gap-3 relative z-10">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg border-2 transition-all duration-500 shadow-lg ${
                ...
                {step > s.number ? <CheckCircle2 size={28} className="text-green-500" /> : s.number}
              </div>
          ...
          {/* Progress Bar Fill */}
          <div 
            className="absolute top-7 left-0 h-1.5 bg-primary -z-10 transition-all duration-1000 rounded-full shadow-[0_0_15px_rgba(0,0,0,0.1)]" 
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-cosmic p-8 md:p-12 relative overflow-hidden group">
        <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 rounded-bl-[10rem] -mr-32 -mt-32 transition-transform group-hover:scale-110 duration-700" />
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="space-y-3">
                ...
                <input required name="customerName" value={formData.customerName} onChange={handleInputChange} type="text" placeholder="Full Legal Name" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary placeholder:text-slate-300" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Husband/Father Name</label>
                <input required name="husbandName" value={formData.husbandName} onChange={handleInputChange} type="text" placeholder="Relative's Name" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary placeholder:text-slate-300" />
              </div>
              ...
              <div className="md:col-span-2 space-y-4">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                  ...
                  Live Photo (Compulsory)
                </label>
                
                <div className="relative group/camera overflow-hidden bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 aspect-video flex flex-col items-center justify-center transition-all hover:border-secondary/30">
                  {showCamera ? (
                    <div className="absolute inset-0 z-20">
                      ...
                      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-4">
                        <button 
                          type="button" 
                          onClick={capture}
                          className="px-8 py-3 bg-secondary text-primary font-black rounded-xl shadow-2xl hover:scale-105 transition-transform flex items-center gap-2"
                        >
                        ...
                        <button 
                          ...
                          className="px-8 py-3 bg-white text-slate-400 font-black rounded-xl shadow-xl hover:scale-105 transition-transform"
                        >
                          CANCEL
                        </button>
                      </div>
                    ...
                    <div className="relative w-full h-full group">
                      <img src={formData.livePhoto} className="w-full h-full object-cover rounded-[2.5rem]" alt="Live" />
                    ...
                    <div className="text-center space-y-4">
                      <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center text-slate-300 shadow-xl mx-auto group-hover/camera:scale-110 group-hover/camera:text-secondary transition-all duration-500">
                        <Camera size={32} />
                      </div>
                      <p className="text-slate-400 font-bold text-sm">Open camera to take a live photo</p>
                      <button 
                        type="button" 
                        onClick={() => setShowCamera(true)}
                        className="px-10 py-4 bg-primary text-secondary rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl active:scale-95"
                      >
                        OPEN CAMERA
                      </button>
                    </div>
              ...
              <div className="md:col-span-2 space-y-3">
                ...
                <textarea required name="permanentAddress" value={formData.permanentAddress} onChange={handleInputChange} rows="3" placeholder="Full Address as per KYC" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary resize-none placeholder:text-slate-300" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Mobile Number</label>
                <div className="relative group/input">
                  <Smartphone className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within/input:text-primary transition-colors" size={20} />
                  <input required name="customerMobile" value={formData.customerMobile} onChange={handleInputChange} type="tel" placeholder="10-digit number" className="w-full pl-14 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary placeholder:text-slate-300" />
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center justify-between">
                  Live Location
                  <button type="button" onClick={handleGetLocation} className="text-[9px] text-secondary hover:underline flex items-center gap-1 font-black">
                    <MapPin size={12} /> Detect
                  </button>
                </label>
                <input required name="liveLocation" value={formData.liveLocation} onChange={handleInputChange} type="text" placeholder="GPS Coordinates" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary placeholder:text-slate-300" />
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="space-y-3">
                ...
                <div className="relative group/input">
                  ...
                  <input required name="loanAmount" value={formData.loanAmount} onChange={handleInputChange} type="number" placeholder="e.g. 50000" className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-black text-primary text-xl placeholder:text-slate-200" />
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Loan Product</label>
                <select name="loanType" value={formData.loanType} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary appearance-none cursor-pointer">
              ...
              <div className="space-y-3">
                ...
                <input required name="purpose" value={formData.purpose} onChange={handleInputChange} type="text" placeholder="e.g. Shop Expansion" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Repayment Schedule</label>
                <select name="repaymentFrequency" value={formData.repaymentFrequency} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary appearance-none cursor-pointer">
          ...
          {step === 3 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="space-y-3">
                ...
                <input required name="bankName" value={formData.bankName} onChange={handleInputChange} type="text" placeholder="Bank Branch Name" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Account Number</label>
                <input required name="bankAccountNo" value={formData.bankAccountNo} onChange={handleInputChange} type="text" placeholder="Full Account Number" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-bold text-primary" />
              </div>
              <div className="md:col-span-2 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="relative">
                    ...
                    <label 
                      ...
                      className={`border-4 border-dashed rounded-[2.5rem] p-8 flex flex-col items-center justify-center gap-3 transition-all cursor-pointer group/upload ${
                        formData.aadharFront ? 'border-green-100 bg-green-50/30' : 'border-slate-50 hover:bg-slate-50/50 hover:border-secondary/20'
                      }`}
                    >
                      {formData.aadharFront ? (
                        ...
                        <>
                          <div className="w-16 h-16 bg-slate-100 rounded-[1.5rem] flex items-center justify-center text-slate-400 group-hover/upload:scale-110 transition-transform duration-500 shadow-xl shadow-slate-50">
                            <Upload size={28} />
                          </div>
                  ...
                  <div className="relative">
                    ...
                    <label 
                      htmlFor="aadharBack"
                      className={`border-4 border-dashed rounded-[2.5rem] p-8 flex flex-col items-center justify-center gap-3 transition-all cursor-pointer group/upload ${
                        formData.aadharBack ? 'border-green-100 bg-green-50/30' : 'border-slate-50 hover:bg-slate-50/50 hover:border-secondary/20'
                      }`}
                    >
                      {formData.aadharBack ? (
                        <>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-10">
            {step > 1 && (
              <button 
                ...
                className="flex-1 py-5 bg-slate-100 text-primary rounded-[1.5rem] font-black tracking-widest text-xs uppercase hover:bg-slate-200 transition-all flex items-center justify-center gap-2"
              >
            ...
            {step < 3 ? (
              <button 
                type="button" 
                onClick={() => setStep(step + 1)}
                className="flex-[2] py-5 bg-primary text-white rounded-[1.5rem] font-black tracking-widest text-xs uppercase hover:bg-slate-900 transition-all shadow-xl shadow-primary/20 flex items-center justify-center gap-2 group"
              >
              ...
              <button 
                ...
                className="flex-[2] py-5 bg-secondary text-white rounded-[1.5rem] font-black tracking-widest text-xs uppercase hover:bg-secondary-dark transition-all shadow-xl shadow-secondary/20 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed group"
              >
                {isSubmitting ? (
                  <>
